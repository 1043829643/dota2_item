...
- `cont(text)` — продолжение/sub-sentence ВНУТРИ `<li>`, мягко присоединённое к основной строке. Для случаев когда патчноут даёт **второе предложение, разъясняющее изменение из первого** (а не отдельное изменение с собственным badge). Использовать через `extra=cont(...)`. Пример: `W(li("All innate abilities now ...", t("REWORK"), extra=cont("Abilities that improve with hero level have base value and increment value...")))`
...
- **Иерархия**: основной change → `cont()` (одно/несколько уточняющих предложений из патча) → `subnote()` (доп. инфо к этим всем)
- `info_li`/raw `<li>` — НЕ использовать; либо bstat_h с note_box, либо subnote, либо cont
