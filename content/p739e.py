...
def build():
    ...
    W(ul_open())
    W(li("Fixed a bug with Aghanim's Scepter that would allow extra Sai abilities to skip cooldown", t("MISC")))
    W(ul_close())
    W(ability("Kazurai Katana", slug="kez_kazurai_katana"))
    W(ul_open())
    W(li("Impale damage changed from post-critical strike bonus damage to a separate instance of physical spell damage", t("REWORK")))
    W(ul_close())
    W(ability("Raptor Dance", slug="kez_raptor_dance"))
    W(ul_open())
    W(li("Max Health as Damage decreased from 3% to 2.5%", b(3, 2.5)))
    W(li("Can no longer be interrupted by casting Grappling Claw", t("MISC")))
    W(ul_close())
    W(ability("Falcon Rush", slug="kez_falcon_rush"))
    W(ul_open())
    W(li("Now has an 825 break distance", t("NEW")))
    W(li("Attack Speed Factor rescaled from 6/8/10/12% to 9%", b([6, 8, 10, 12], 9)))
    W(ul_close())
    W(ability("Shodo Sai", slug="kez_shodo_sai"))
    W(ul_open())
    W(li("Mark Critical Strike decreased from 140/160/180/200% to 125/150/175/200%", b([140, 160, 180, 200], [125, 150, 175, 200])))
    W(ul_close())
