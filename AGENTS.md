...
- `b(old, new, l=False)` — числовой бейдж в %. `l=True`: меньше = лучше (cooldown, mana cost, BAT, gold cost, penalty, channel time, recharge, cast point)
  ...
  - **Общий тег — по МАКС-уровню** (последний ненулевой ранг). **Авто-уточнение:** если макс-ранг — **мелкий нерф (≤12%)**, а среднее знаковое % положительное (ранние баффы перевешивают незначительный поздний минус) → авто **бафф** (Riki Blink Strike 15/30/45/60→25/35/45/55 = +67/+17/0/−8). Порог ≤12% оставляет «сплющивания» (X/Y/Z/W → одно значение, большой минус на максе, напр. Drow Agility 4/8/12/16→10 = +150/+25/−17/−38) нерфом. Обратный случай (ранний нерф/поздний бафф, Disseminate) не затронут. Вручную `force_overall="buff"/"nerf"` — переопределение. Пер-уровневые % не меняются.
...
- **`terrain.html`** (`build_terrain.py`, 5-я вкладка Materials) — сравнение рельефа old→new через **шторку-слайдер** (две карты `icons/maps/map_<ver>.webp`, попиксельно совмещённые, клип через `clip-path` по `--pos`) + список Terrain Changes этого патча. Полный план и TODO — `docs/terrain.md`.
  - **Список изменений парсится из `build_patch.py`** (никакого дублирования): `_terrain_changes_by_patch()` читает каждую секцию `plain_header("Terrain Changes")` → `{ver: [(text, TAG)]}`. Сейчас это 7.41 и 7.40 (у 7.40 свой большой ремап-список — не путать). Тег `b(...)`-строк выводится по направлению с учётом `l=True` (дешевле mana cost = BUFF).
  ...
  - **Карта-пара есть только для патчей из `_MAP_PAIRS`** (сейчас `7.41`→слайдер). Для остальных (7.40) рисуется `_fallback_html` — последняя карта в блюре + оверлей «Map comparison for X isn't available yet», список изменений при этом остаётся.
