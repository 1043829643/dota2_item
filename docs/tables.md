...
### Hero Lab battle rules

- Both heroes make their first attack at `0.00s`; attack interval applies between subsequent attacks.
- Every actual melee hero has hidden physical damage block: 16 damage with 50% long-run chance, implemented as PRD with a `30.21%` increment (`P(4) = 100%`). The PRD failure counter resets after a proc.
- Damage block is subtracted before armor. If multiple damage-block sources proc, only the highest block value applies.
- Ranged heroes do not receive hidden melee block. Vengeful Spirit is an explicit exception even when her attack-type interactions are treated as melee: `vengefulspirit` never receives the hidden block.
...
`EHP phys`, `EHP mag`, `Armor %`, `Dmg min`, `Dmg max`, `Attack Interval`,
projectile speed, turn rate, collision size, and bound radius.
...
- **Fit-to-width / latest flush right:** `dynLayoutMatrix` shows the most-recent patches that fit
  ...
  of cells). "Hide old" OFF shows every patch column (scroll, parked at the right so the latest stays in view).
